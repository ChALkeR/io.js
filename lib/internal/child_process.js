...
function optionsHarden(options) {
  if (!hardened) return;
  const { args, file, shell, detached } = options;
  const info = { args: [...args], file, shell, detached };
  Object.freeze(info);
  Object.freeze(info.args);
  if (!hardened.validator(info)) throw new Error('TODO: define an error type');
}
...
function harden(options) {
  if (!options.validator) new ERR_MISSING_ARGS('options.validator');
  hardened = {
    validator: options.validator
  };
  Object.freeze(module.exports);
  Object.freeze(ChildProcess.prototype);
  for (const method of Object.values(module.exports)) Object.freeze(method);
}
